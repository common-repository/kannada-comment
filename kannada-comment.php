...
Plugin URI: http://www.prasannasp.net/wordpress-plugins/kannada-comment/
...
function kncm_enqueue_scripts() {
	if( is_singular() && comments_open() ) {
  wp_enqueue_script( 'google-jsapi', '//www.google.com/jsapi', array(), '', false );
  wp_enqueue_script( 'kannada-comment-js', plugins_url( '/kncm.js' , __FILE__ ), array(), '', false );
  	}
}
add_action( 'wp_enqueue_scripts', 'kncm_enqueue_scripts' );

// echo kncmlang variable in head
function kncm_lang_var() {
	$options = get_option('kncm_options');
	$language = $options['lang_select'];
?>
<script type="text/javascript">      
        var kncmlang = <?php echo $language; ?>;
</script>
<?php
}
add_action( 'wp_print_scripts', 'kncm_lang_var' );
...
function kannada_comment_switch_hook() {
	...
	switch ($switchplace) {
		...
		case 'comment_form' : add_action('comment_form', 'kannada_comment_switch');
			break;

		case 'comment_form_before' : add_action('comment_form_before', 'kannada_comment_switch');
			break;

		case 'comment_form_after' : add_action('comment_form_after', 'kannada_comment_switch');
			break;

		default : add_action('comment_form_top', 'kannada_comment_switch');
	}
}
